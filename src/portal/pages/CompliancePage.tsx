...
export default function CompliancePage() {
  const { compliance, isLoading } = useData();
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [uploadingItem, setUploadingItem] = useState<string | null>(null);

  const comp = compliance || { completed: 0, total: 6, requirements: [], kycStatus: 'pending', accreditationStatus: 'pending' };
  const requirements = comp.requirements || [];
  
  const filteredRequirements = selectedCategory
    ? requirements.filter((r: any) => r.category === selectedCategory)
    : requirements;

  const completionPercentage = comp.total > 0 ? (comp.completed / comp.total) * 100 : 0;
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/50 text-sm">{comp.completed} of {comp.total} requirements completed</p>
            </div>
          </div>
          ...
          <div className="flex-1 lg:border-l lg:border-[#2A2A2A] lg:pl-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-white/40 mb-1">KYC Status</p>
                <p className="text-white capitalize">{comp.kycStatus}</p>
              </div>
              <div>
                <p className="text-sm text-white/40 mb-1">Accreditation</p>
                <p className="text-white capitalize">{comp.accreditationStatus}</p>
              </div>
            </div>
          </div>
          ...
          {completionPercentage < 100 && (
            <div className="flex items-center gap-2 px-4 py-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
              ...
              <span className="text-sm text-yellow-500">{comp.total - comp.completed} items require attention</span>
            </div>
          )}
        </div>
      </div>
                  ...
                  <div>
                    ...
                    {req.uploadedFiles?.length > 0 && (
                      <div className="mt-4 space-y-2">
                        {req.uploadedFiles.map((file: string, i: number) => (
                          <div 
                            key={i}
      ...
      <div className="bg-[#1A1A1A] rounded-xl p-6">
        ...
        <p className="text-white/50 text-sm mb-4">Our compliance team is available to assist you with any KYC/AML documentation questions.</p>
      ...
      {uploadingItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => setUploadingItem(null)} />
          <div className="relative bg-[#141414] border border-[#2A2A2A] rounded-xl w-full max-w-md p-6">
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setUploadingItem(null)} className="flex-1 px-4 py-3 border border-[#2A2A2A] rounded-lg text-white/70 hover:text-white transition-colors">
                Cancel
              </button>
              <button onClick={() => setUploadingItem(null)} className="flex-1 px-4 py-3 bg-[#8FB8A3] text-[#0A0A0A] rounded-lg font-medium hover:bg-[#7BA391] transition-colors">
                Upload Document
              </button>
            </div>
          </div>
